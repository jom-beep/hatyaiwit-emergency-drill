...
## โหมดสาธิต (ไม่ต้องล็อกอิน Google)

เปิด `/demo` หรือ `/?demo=1` เพื่อเดินดูวันซ้อมด้วยข้อมูลจำลองในเบราว์เซอร์

- เข้าได้จากหน้าเข้าสู่ระบบที่ปุ่ม **ดูโหมดสาธิต — ไม่ต้องเข้าสู่ระบบ**
- ใช้ผู้ใช้ โซน อุปกรณ์ และการฝึกซ้อมปลอมในเครื่องผู้ชมเท่านั้น ไม่เรียก `/api/*` จริง ไม่ข้าม Google OAuth ของระบบจริง และไม่ส่ง Web Push
- แถบเหลืองบอกชัดว่าเป็นข้อมูลจำลอง ทุกแผงมีป้าย **นี่คือการฝึกซ้อม · DRILL**
- สลับหน้าจอหลักได้ 2 แบบ: **ศูนย์ควบคุม** และ **ผู้รับแจ้ง** พร้อมลำดับสาธิตบนจอ เปิด 2 แท็บคนละบทบาท หมุดกับเขตแดงซิงก์กันในเครื่องนี้
- ปุ่มเริ่มซ้อมหลักคือ **กราดยิง / ตรีจักร 191** เท่านั้น เทมเพลตอื่น (เพลิงไหม้ / แผ่นดินไหว / ล็อกดาวน์ / ภัยคุกคาม) อยู่ใน «เทมเพลตอื่น»
- คนที่ 2–5 ของคิว 20 วินาที อยู่ใน «ทดสอบคิวแอดมิน (20 วิ)» ไม่ได้อยู่บนเส้นทางสาธิตหลัก การยุติยังต้องยืนยัน 2 คน
- การ์ดคำแนะนำสั้นใต้ปุ่มใหญ่ตามประเภทเหตุ (เช่น ล็อกดาวน์: เงียบ ปิดไฟ ห้ามเปิดประตู)
- แผนผังโรงเรียนจำลอง: ผู้รับแจ้งปักหมุดพร้อมเวลา ศูนย์ควบคุมวางเขตแดงได้หลังยืนยันควบคุม มีกระดานคนที่ยังไม่ตอบตามอาคาร (คนที่กดไม่อยู่ในพื้นที่ไม่ถูกนับเป็นผู้ไม่ตอบ) และสรุปหลังซ้อม
- หน้า `/demo` เปิดที่ขั้น «เริ่มซ้อม» ไม่ได้พาเข้าเหตุที่กำลังดำเนินอยู่ หมุดและเขตแดงซิงก์ข้ามแท็บผ่าน localStorage / BroadcastChannel ของโหมดสาธิต (ไม่เรียก Durable Object หรือ `/api/*` จริง) — ยุติการฝึกแล้วยังต้องยืนยัน 2 คน และจะล้างหมุดกับเขตแดงอัตโนมัติ

เมื่อปิดโหมดสาธิต เส้นทาง `/` ยังต้องเข้าสู่ระบบด้วยบัญชี `@khanchai.ac.th` ตามเดิม
